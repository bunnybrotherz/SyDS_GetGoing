...
var perishedCount = { youth: 0, adult: 0, elderly: 0, PWD: 0 };
...
var victimSpeeds = {
  ...
  elderly: 0.4,  // Much slower
  PWD: 0.3, // Even slower
  helper: 0.5   // Helpers are fast
};

// NEW: First aid priority bias (0-100)
// 100 means helpers will always prioritize first aid victims regardless of distance
// 0 means pure distance-based assignment with no bias
var firstAidPriorityBias = 100;
  ...
  agents.forEach((agent) => {
      ...
      if (agent.isHelper && !agent.stayingAtGoal) {
        ...
        if (timer <= 5) {
          ...
          if (unassignedVictims.length > 0) {
            ...
            console.log(`Helper redeployed to help ${targetVictim.type} victim needing ${targetVictim.helpNeeded} (priority score: ${potentialVictims[0].score.toFixed(1)})`);
          }
        }
      }
      return;
    ...
    if (agent.isHelper && agent.assignedVictim) {
      ...
      if (!reachedVictim) {
        ...
        // Single wait duration now based on help needed
        let waitDuration = 0;
...
function moveTowards(agent, target) {
  ...
  // Improved tolerance check to prevent bouncing
...
// NEW: Function to calculate priority score with bias for first aid victims
function calculatePriorityScore(helper, victim) {
  // Base score is inverse of distance (closer = higher score)
  const distance = calculateDistance(helper, victim);
  let score = 100 - Math.min(distance * 5, 95); // Convert distance to 5-100 scale
  
  // Apply first aid priority bias
  if (victim.helpNeeded === 'first_aid') {
    // Add bonus based on priority bias (0-100)
    score += firstAidPriorityBias;
  }
...
// Function to generate random agents - modified to prioritize first aid victims
function generateRandomAgents(count) {
  ...
  for (let i = 0; i < victimsCount; i++) {
    ...
    let helpNeeded;
    if (victimType === 'PWD' || victimType === 'elderly') {
      // Elderly and PWD people more likely to need first aid
  ...
  // NEW: Calculate priority scores for all victims
  const scoredVictims = victims.map(victim => ({
    victim: victim,
    scores: helpers.map(helper => ({
      helper: helper,
      score: calculatePriorityScore(helper, victim)
    }))
  }));

  // NEW: Sort victims by their highest possible score (highest first)
  // This ensures highest priority victims get assigned first
  scoredVictims.sort((a, b) => {
    const maxScoreA = Math.max(...a.scores.map(s => s.score));
    const maxScoreB = Math.max(...b.scores.map(s => s.score));
    return maxScoreB - maxScoreA;
  });

  // NEW: Track which helpers have been assigned
  const assignedHelpers = new Set();

  // NEW: For each victim in priority order, find the best available helper
  scoredVictims.forEach(scoredVictim => {
    // Skip if all helpers are assigned
    if (assignedHelpers.size >= helpers.length) return;
    
    // Get scores for this victim, but only for unassigned helpers
    const availableScores = scoredVictim.scores.filter(s => !assignedHelpers.has(s.helper));
    
    if (availableScores.length === 0) return;
    
    // Sort helper scores (highest first)
    availableScores.sort((a, b) => b.score - a.score);
    
    // Assign the best available helper to this victim
    const bestMatch = availableScores[0];
    const helper = bestMatch.helper;
    const victim = scoredVictim.victim;
    
    helper.assignedVictim = victim;
    victim.assignedHelper = helper;
    
    // Mark this helper as assigned
    assignedHelpers.add(helper);
    
    console.log(`Assigned helper to ${victim.type} victim needing ${victim.helpNeeded} (priority score: ${bestMatch.score.toFixed(1)}, distance: ${calculateDistance(helper, victim).toFixed(2)})`);
  });
...
function renderAgents() {
  ...
  surface.selectAll("line.assignment")
    ...
    .attr("stroke", d => {
      // Color based on help needed type
      if (d.assignedVictim.helpNeeded === "packing") return "rgba(54, 162, 235, 0.7)";  // Blue for packing
      else return "rgba(255, 99, 132, 0.7)";  // Red for first aid
    })
...
// Update logAssignmentDetails to include priority information
function logAssignmentDetails() {
  ...
  console.log(`PWD victims: ${agents.filter(a => a.isVictim && a.type === 'PWD').length}`);
  ...
  console.log(`First Aid Priority Bias: ${firstAidPriorityBias}%`);
  ...
  helpers.forEach((helper, index) => {
    if (helper.assignedVictim) {
      ...
      const score = calculatePriorityScore(helper, helper.assignedVictim).toFixed(1);
      console.log(`Helper ${index+1} → Victim type: ${helper.assignedVictim.type}, Needs: ${helper.assignedVictim.helpNeeded}, Distance: ${distance}, Priority Score: ${score}`);
    } else {
  ...
  // Count and log assignments by help type
  const firstAidAssignments = agents.filter(a => a.isVictim && a.assignedHelper && a.helpNeeded === 'first_aid').length;
  const packingAssignments = agents.filter(a => a.isVictim && a.assignedHelper && a.helpNeeded === 'packing').length;
  
  console.log(`First aid victims assigned helpers: ${firstAidAssignments}/${totalByHelpNeeded.first_aid} (${totalByHelpNeeded.first_aid > 0 ? Math.round(firstAidAssignments/totalByHelpNeeded.first_aid*100) : 0}%)`);
  console.log(`Packing victims assigned helpers: ${packingAssignments}/${totalByHelpNeeded.packing} (${totalByHelpNeeded.packing > 0 ? Math.round(packingAssignments/totalByHelpNeeded.packing*100) : 0}%)`);
}

// UPDATED: Show help needed type stats
function updateCounters() {
  ...
  document.getElementById('urgency-stats').innerHTML = `
    ...
    <div class="urgency-progress">
      <div>Packing Victims: ${packingVictims.saved}/${packingVictims.total} (${packingVictims.total > 0 ? Math.round(packingVictims.saved/packingVictims.total*100) : 0}%)</div>
      <div class="progress-bar">
        <div class="progress-fill" style="width: ${packingVictims.total > 0 ? Math.round(packingVictims.saved/packingVictims.total*100) : 0}%; background-color: rgba(54, 162, 235, 0.7);"></div>
      </div>
    </div>
    <div class="urgency-progress">
      <div>First Aid Victims: ${firstAidVictims.saved}/${firstAidVictims.total} (${firstAidVictims.total > 0 ? Math.round(firstAidVictims.saved/firstAidVictims.total*100) : 0}%)</div>
      <div class="progress-bar">
        <div class="progress-fill" style="width: ${firstAidVictims.total > 0 ? Math.round(firstAidVictims.saved/firstAidVictims.total*100) : 0}%; background-color: rgba(255, 99, 132, 0.7);"></div>
      </div>
    </div>
  `;
}
...
function startButtonHandler() {
    ...
    perishedCount = { youth: 0, adult: 0, elderly: 0, PWD: 0 };
    totalAgentsByType = { youth: 0, adult: 0, elderly: 0, PWD: 0 };
...
function showResults() {
  ...
  let modalHTML = `
    ...
    <div class="result-category">
      <div class="result-label">PWD: ${results.PWD}/${totalAgentsByType.PWD || 0}</div>
      <div class="result-bar-container">
        <div class="result-bar pwd-bar" style="width: ${totalAgentsByType.PWD > 0 ? Math.round((results.PWD/totalAgentsByType.PWD)*100) : 0}%"></div>
      </div>
      <div class="result-percentage">${totalAgentsByType.PWD > 0 ? Math.round((results.PWD/totalAgentsByType.PWD)*100) : 0}%</div>
    </div>
  ...
  modalHTML += `
    ...
    <div class="urgency-progress" style="margin-top: 10px;">
      <div><strong>First Aid Priority: ${firstAidPriorityBias}%</strong></div>
    </div>
...
function initScenario() {
    ...
    timer = 30;
    savedCount = { youth: 0, adult: 0, elderly: 0, PWD: 0, helper: 0 };
    savedByHelpType = { packing: 0, first_aid: 0 };
    perishedCount = { youth: 0, adult: 0, elderly: 0, PWD: 0 };
    totalAgentsByType = { youth: 0, adult: 0, elderly: 0, PWD: 0 };
    totalByHelpNeeded = { packing: 0, first_aid: 0 };
    isRunning = false;
  
    // Set up the simulation elements
    setupCanvas();
  }
  ...
  // Replace the window.onload with the initialization function
