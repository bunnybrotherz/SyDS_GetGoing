...
var victimImages = {
  ...
  helper: "images/first-aider.png" // New single helper type
};
...
var isRunning = false;
var timer = 30;
var savedCount = { youth: 0, adult: 0, elderly: 0, disabled: 0, helper: 0 }; // Updated helper count
var perishedCount = { youth: 0, adult: 0, elderly: 0, disabled: 0, helper: 0 }; // Track perished people
...
// Ratio of victims to helpers (1:2 by default)
var victimToHelperRatio = 2;

// Speed multipliers for different victim types - Updated speeds
var victimSpeeds = {
  youth: 0.12,    // Updated speed
  adult: 0.11,    // Updated speed
  elderly: 0.10,  // Updated speed
  disabled: 0.09, // Updated speed
  helper: 0.11    // Helper speed (fast like youth)
};

// UPDATED: Changed urgency levels to prioritize adults and youth, and combine elderly/disabled
var urgencyLevels = {
  youth: 1,    // High urgency
  adult: 1,    // High urgency
  elderly: 2,  // Medium urgency (combined with disabled)
  disabled: 2  // Medium urgency (combined with elderly)
};
...
// Modified updateAgents function to handle both need types
...
function moveToGoal(helper, victim) {
  ...
  if (Math.abs(goal.col - victim.col) <= epsilon && Math.abs(goal.row - victim.row) <= epsilon) {
    ...
    if (!victim.counted) {
      ...
      savedCount["helper"]++; // Now just a single helper type
      victim.counted = true;
      helper.counted = true;
...
function getRandomAgentType() {
  ...
  if (rand < (1 / (1 + victimToHelperRatio))) {
    return 'helper'; // Now just a single helper type
  } else {
...
// Modified function to spawn new helpers with only one helper type
function spawnNewHelpers() {
  ...
  if (unassignedVictims.length > 0) {
    ...
    newHelpers.forEach(helper => {
      ...
      console.log(`New helper spawned and assigned to ${targetVictim.type} victim (urgency: ${targetVictim.urgencyLevel}, distance: ${potentialVictims[0].distance.toFixed(2)})`);
    });
...
document.getElementById("start-button").addEventListener("click", () => {
  cleanupTimers();
  timer = 30;
  savedCount = { youth: 0, adult: 0, elderly: 0, disabled: 0, helper: 0 }; // Updated for single helper
  perishedCount = { youth: 0, adult: 0, elderly: 0, disabled: 0, helper: 0 }; // Reset perished count
...
// Updated generateRandomAgents to use a single helper type and assign need types to victims
function generateRandomAgents(count) {
  ...
  availableHelpers.forEach(helper => {
    ...
    console.log(`Assigned helper to high-priority ${closestVictim.type} victim for ${victimNeedType[closestVictim.id]} (distance: ${victimDistances[0].distance.toFixed(2)})`);
  });
  ...
  if (remainingHelpers.length > 0 && highPriorityVictims.length === 0) {
    ...
    // Create array of medium priority victims (elderly AND disabled now)
    const mediumPriorityVictims = [...victimsByType.disabled, ...victimsByType.elderly];
    ...
    remainingHelpers.forEach(helper => {
      ...
      console.log(`Assigned helper to medium-priority ${closestVictim.type} victim for ${victimNeedType[closestVictim.id]} (distance: ${victimDistances[0].distance.toFixed(2)})`);
    });
  }
...
function calculatePerished() {
  ...
  perishedCount = { youth: 0, adult: 0, elderly: 0, disabled: 0, helper: 0 }; // Updated for single helper
  ...
  agents.filter(a => a.isHelper && !a.reachedGoal).forEach(helper => {
    perishedCount.helper++; // Updated for single helper
  });
  
  return perishedCount;
}

// Modify renderAgents function for unified helper type
function renderAgents() {
  ...
  surface.selectAll("line.assignment")
    ...
    .attr("stroke", d => {
      ...
      else return "rgba(80, 255, 64, 0.72)"; // Elderly and Disabled - Green
    })
  ...
  surface.selectAll("circle.urgency")
    ...
    .attr("fill", d => {
      ...
      else return "rgba(255, 159, 64, 0.3)"; // Elderly and Disabled
    });
...
// Update logAssignmentDetails for unified helper type
function logAssignmentDetails() {
  ...
  console.log(`Medium urgency victims: ${victimsByUrgency[2].length} (Elderly & Disabled combined)`);
  ...
  // Updated warning to check for high priority victims without helpers
...
// Update the counters for unified helper type
function updateCounters() {
  ...
  document.getElementById('helper-count').textContent = `Helpers: ${savedCount.helper}`; // Update for single helper
  document.getElementById('timer-count').textContent = `Time: ${timer}s`;
  
  // Update progress bars in the urgency stats section - Removed low priority, now just high and medium
...
// Update the showResults function for unified helper
function showResults() {
  ...
  const results = {
    ...
    helper: savedCount.helper // Updated for unified helper
  };
...
// Update the startTimer function to use showResults
function startTimer() {
