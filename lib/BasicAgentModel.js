...
var victimSpeeds = {
  youth: 0.25,    // Faster than adults
  adult: 0.15,    // Normal speed
  elderly: 0.08,  // Much slower
  disabled: 0.06, // Even slower
  helper: 0.15   // Helpers are fast
};
...
function spawnNewHelpers() {
  ...
  if (unassignedVictims.length > 0) {
    // For each helper, prioritize first aid victims based on priority bias
    newHelpers.forEach(helper => {
      // Skip if no unassigned victims left
      if (unassignedVictims.length === 0) return;
      
      // NEW: Calculate priority scores that combine distance and type priority
      ...
      console.log(`New helper spawned and assigned to ${targetVictim.type} victim needing ${targetVictim.helpNeeded} (priority score: ${potentialVictims[0].score.toFixed(1)}, distance: ${potentialVictims[0].distance.toFixed(2)})`);
    });
  }
...
function updateCounters() {
  ...
  document.getElementById('urgency-stats').innerHTML = `
    ...
    <div class="urgency-progress">
      <div>First Aid Priority: ${firstAidPriorityBias}%</div>
    </div>
  `;
}
