...
function updateVictims() {
  victims.forEach(v => {
    ...
    if (Math.abs(goal.col - v.col) > epsilon) {
      v.col += Math.sign(goal.col - v.col) * speed;
    }
    if (Math.abs(goal.row - v.row) > epsilon) {
      v.row += Math.sign(goal.row - v.row) * speed;
    }
  ...
  groups.forEach(group => {
    for (let i = 0; i < group.count; i++) {
      positions.push({
        ...
        type: group.type,
        saved: false,
        startDelay: Math.random() < 0.5 ? 3 : 0  // 50% chance to wait 3s
      });
    }
  });
