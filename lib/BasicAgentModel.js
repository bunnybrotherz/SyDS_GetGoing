...
var urgencyLevels = {
  ...
  elderly: 2,  // Low urgency
  disabled: 2  // Low urgency
};
...
// CHANGE: Optimization variables
const updateFrequency = 5; // Only update positions every 5 timesteps
const agentBatchSize = 10000; // Process 10,000 agents per batch
let currentAgentBatch = 0; // Track which batch we're processing
let updateCounter = 0; // Counter for update frequency
...
function runSimulation(simulationSpeedMultiplier = 1000) {
  ...
  while (simulationTime < maxSimulationTime && !areAllVictimsEscorted()) {
    // CHANGE: Implement reduced calculation frequency
    if (updateCounter % updateFrequency === 0) {
      // CHANGE: Implement progressive simulation (process agents in batches)
      const startIdx = currentAgentBatch * agentBatchSize;
      const endIdx = Math.min(startIdx + agentBatchSize, agents.length);
      
      // Update just this batch of agents
      for (let i = startIdx; i < endIdx; i++) {
        updateSingleAgent(agents[i], timeStep * updateFrequency);
      }
      
      // Move to next batch, looping back to start if needed
      currentAgentBatch = (currentAgentBatch + 1) % Math.ceil(agents.length / agentBatchSize);
      
      // Only check for helper reassignment if we've processed all agents
      if (currentAgentBatch === 0) {
        reassignHelpers();
      }
    }
  ...
  // Log helper reassignment statistics
  console.log("=== HELPER REASSIGNMENT STATISTICS ===");
  const helperCounts = {};
  agents.filter(a => a.isHelper).forEach(helper => {
    const count = helper.victimsHelped;
    helperCounts[count] = (helperCounts[count] || 0) + 1;
  });

  console.log("Number of victims helped | Number of helpers");
  Object.keys(helperCounts).sort((a, b) => parseInt(a) - parseInt(b)).forEach(count => {
    console.log(`${count} victim(s) | ${helperCounts[count]} helpers`);
  });

  // Calculate average victims helped per helper
  const totalHelpers = agents.filter(a => a.isHelper).length;
  const totalVictimsHelped = agents.filter(a => a.isHelper).reduce((sum, helper) => sum + helper.victimsHelped, 0);
  const avgVictimsHelped = totalVictimsHelped / totalHelpers;
  console.log(`Average victims helped per helper: ${avgVictimsHelped.toFixed(2)}`);
...
// CHANGE: New function to update a single agent
function updateSingleAgent(agent, deltaTime) {
  ...
  } else if (agent.isVictim) {
    // CHANGE: Allow unassigned victims to move on their own after 1 hour
    if (!agent.assignedHelper && simulationTime >= 450) {
...
// CHANGE: No longer needed since we now use updateSingleAgent
// function updateAgents(deltaTime) {
//   // This function is now replaced by updateSingleAgent which processes agents one by one
// }
...
// CHANGE: Add new function for victims to move to goal on their own
function moveTowardsGoal(agent, deltaTime) {
  let speed = baseVictimSpeeds[agent.type];
...
// Modified to mark agents as permanently at goal or just reached goal
...
function generateRandomAgents() {
  ...
  // Create high priority victims (youth and adults)
  const highPriorityVictims = [...victimsByType.youth, ...victimsByType.adult];
  
  // Create low priority victims (disabled and elderly)
  const lowPriorityVictims = [...victimsByType.disabled, ...victimsByType.elderly];
  
  // For each helper, find the nearest high priority victim first
  availableHelpers.forEach(helper => {
  ...
  // Assign remaining helpers to low priority victims
  if (remainingHelpers.length > 0 && lowPriorityVictims.length > 0) {
    console.log("Assigning remaining helpers to low priority victims");
...
function addAdvancedSettings() {
    ...
    <div id="advanced-settings" style="display: none; margin-bottom: 20px;">
     ...
     <h4>Urgency Levels (1-2, where 1 is high priority)</h4>
      ...
      <div style="display: grid; grid-template-columns: auto auto; gap: 10px;">
        ...
        <input type="number" id="update-freq" value="${updateFrequency}" min="1" max="20" step="1">
        
        <label for="batch-size">Agent Batch Size:</label>
        <input type="number" id="batch-size" value="${agentBatchSize}" min="1000" max="50000" step="1000">
      </div>
  ...
  document.getElementById('update-advanced').addEventListener('click', function() {
    ...
    updateFrequency = parseInt(document.getElementById('update-freq').value) || 5;
    agentBatchSize = parseInt(document.getElementById('batch-size').value) || 10000;
...
function displayResults() {
  ...
  if (simulationResults.length > 0) {
    ...
    Object.keys(helperCounts).sort((a, b) => parseInt(a) - parseInt(b)).forEach(count => {
      const number = helperCounts[count];
      const percentage = ((number / totalHelpers) * 100).toFixed(1);
      
      resultText += `
        <tr>
          <td>${count} victim${count === "1" ? "" : "s"}</td>
          <td>${number}</td>
          <td>${percentage}%</td>
        </tr>`;
    });
    
    // Add average victims helped
    const totalVictimsHelped = agents.filter(a => a.isHelper).reduce((sum, helper) => sum + helper.victimsHelped, 0);
    const avgVictimsHelped = totalVictimsHelped / totalHelpers;
    ...
    resultText += `
      <tr style="font-weight: bold; background-color: #f0f0f0;">
        <td>Average</td>
        <td colspan="2">${avgVictimsHelped.toFixed(2)} victims per helper</td>
      </tr>`;
