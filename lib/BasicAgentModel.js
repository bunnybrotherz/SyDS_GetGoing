...
var perishedCount = { youth: 0, adult: 0, elderly: 0, disabled: 0, helper: 0 };
...
function spawnNewHelpers() {
  ...
  if (unassignedVictims.length > 0) {
    ...
    newHelpers.forEach(helper => {
      ...
      // Calculate urgency and distance for all victims
...
// Dynamic random agent generation based on urgency levels
function generateRandomAgents(count) {
  ...
  // Group victims by urgency level for assignment
  const victimsByUrgency = {};
  Object.values(urgencyLevels).forEach(level => {
  ...
  // Get all urgency levels, sorted with highest priority first
  const priorityLevels = [...new Set(Object.values(urgencyLevels))].sort();
  
  // Assign helpers to victims in order of priority
  priorityLevels.forEach(level => {
    console.log(`Assigning helpers to urgency level ${level} victims...`);
    
    // Get victims with this urgency level
    const currentVictims = victimsByUrgency[level];
    
    // For each helper, find the nearest victim at this urgency level
    availableHelpers.forEach(helper => {
      // Skip if helper already assigned or no victims at this level
      if (helper.assignedVictim || currentVictims.length === 0) return;
      
      // Calculate distances to all victims at this urgency level
      const victimDistances = currentVictims.map(victim => {
        return {
          victim: victim,
          distance: calculateDistance(helper, victim)
        };
      ...
      const index = currentVictims.indexOf(closestVictim);
      if (index > -1) {
        currentVictims.splice(index, 1);
      }
      
      const victimTypes = getVictimTypesByUrgencyLevel(level);
      console.log(`Assigned helper to ${closestVictim.type} victim (urgency ${level}: ${victimTypes.join(', ')}) for ${victimNeedType[closestVictim.id]} (distance: ${victimDistances[0].distance.toFixed(2)})`);
    });
  });
...
function showResults() {
  ...
  let modalHTML = `
    ...
    <div class="stat-box ${getUrgencyClass(urgencyLevels.disabled)}">PWD: ${results.disabled} (${totalAgentsByType.disabled - results.disabled} perished)</div>
  ...
  document.getElementById('close-modal').addEventListener('click', () => {
    try {
      ...
      // Make sure all timers are stopped completely
      clearInterval(simTimer);
      clearInterval(countdownTimer);
      clearTimeout(helperSpawnTimer);
      simTimer = null;
      ...
      isRunning = false; // Ensure simulation is marked as not running
    } catch (e) {
