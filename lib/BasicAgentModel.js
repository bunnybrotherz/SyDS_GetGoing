...
function generateRandomAgents(count) {
  ...
  const victimsByUrgency = {
    1: [], // High urgency (now youth and adults)
    2: [], // Medium urgency (now disabled)
    3: []  // Low urgency (now elderly)
  };
  ...
  // First assign helpers to high urgency victims (now youth and adults) based on proximity
  ...
  // Then assign helpers to medium urgency victims (now disabled)
  ...
  // Finally assign remaining helpers to low urgency victims (now elderly)
...
// Update the rendering and UI functions to match our new modern UI
...
function setupCanvas() {
  ...
  // Updated text in counter container
  counterContainer.innerHTML = `
    <h3>Victim Priority Levels</h3>
    <p id="youth-count">Youth Saved: 0 (High Urgency)</p>
    <p id="adult-count">Adults Saved: 0 (High Urgency)</p>
    <p id="disabled-count">Disabled Saved: 0 (Medium Urgency)</p>
    <p id="elderly-count">Elderly Saved: 0 (Low Urgency)</p>
    ...
    <div id="urgency-stats"></div>
  `;

  const legend = document.createElement("div");
  legend.id = "urgency-legend";
  // Updated legend text
  legend.innerHTML = `
    <div style="margin-top: 10px;">
      <span style="background-color: rgba(255, 0, 0, 0.3); padding: 2px 10px; margin-right: 5px;">High Urgency (Youth & Adults)</span>
      <span style="background-color: rgba(255, 165, 0, 0.3); padding: 2px 10px; margin-right: 5px;">Medium Urgency (Disabled)</span>
      <span style="background-color: rgba(0, 128, 0, 0.3); padding: 2px 10px;">Low Urgency (Elderly)</span>
    </div>
  `;
  ...
  testButton.textContent = "Test with 20 Agents";
