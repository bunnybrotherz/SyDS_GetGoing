...
function generateRandomAgents(count) {
  ...
  // Generate victims with different types
  const victims = [];
  ...
  for (let i = 0; i < helpers.length; i++) {
    ...
    // Find the nearest unassigned victim
    for (let j = 0; j < victims.length; j++) {
      if (!victims[j].assignedHelper) {
        const distance = Math.sqrt(
          Math.pow(victims[j].col - helpers[i].col, 2) + 
          Math.pow(victims[j].row - helpers[i].row, 2)
        );

        if (distance < minDistance) {
          minDistance = distance;
          nearestVictim = victims[j];
        }
      }
    }

    // Assign the helper to the victim and vice versa
    if (nearestVictim) {
      helpers[i].assignedVictim = nearestVictim;
      nearestVictim.assignedHelper = helpers[i];
      console.log(`Assigned ${helpers[i].type} helper to ${nearestVictim.type} victim`);
    }
  }
