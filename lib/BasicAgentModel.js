...
// Update moveTowards and other functions by removing urgency-related logic
function spawnNewHelpers() {
  ...
  // If we have unassigned victims, assign them to our new helpers based on matching type first, then distance
if (unassignedVictims.length > 0) {
  // For each helper, find the nearest victim that needs their type of help
  newHelpers.forEach(helper => {
    // Skip if no unassigned victims left
    if (unassignedVictims.length === 0) return;
    
    // First, filter victims by those who need this helper's type of help
    const matchingVictims = unassignedVictims.filter(victim => victim.helpNeeded === helper.type);
    
    // If we found matching victims, use them, otherwise use all unassigned victims
    const victimsToConsider = matchingVictims.length > 0 ? matchingVictims : unassignedVictims;
    
    // Calculate distances
    const potentialVictims = victimsToConsider.map(victim => {
      return {
        victim: victim,
        distance: calculateDistance(helper, victim),
        isMatch: victim.helpNeeded === helper.type
      };
    });
    
    // Sort by match first, then by distance
    potentialVictims.sort((a, b) => {
      // If one is a match and the other isn't, prioritize the match
    ...
    // Assign this helper to the best victim
    ...
    const matchStatus = potentialVictims[0].isMatch ? "MATCH" : "MISMATCH";
    console.log(`New ${helper.type} helper spawned and assigned to ${targetVictim.type} victim needing ${targetVictim.helpNeeded} (${matchStatus}, distance: ${potentialVictims[0].distance.toFixed(2)})`);
  });
}
...
// Function to generate random agents - modified to remove urgency
function generateRandomAgents(count) {
  ...
  for (let i = 0; i < victimsCount; i++) {
    ...
    if (victimType === 'disabled' || victimType === 'elderly') {
      ...
      helpNeeded = Math.random() < 0.7 ? 'first_aider' : 'packing';
    } else if (victimType === 'youth') {
      // Youth more likely to need packing help
      helpNeeded = Math.random() < 0.7 ? 'packing' : 'first_aider';
    } else {
      // Adults have equal chance
      helpNeeded = Math.random() < 0.5 ? 'packing' : 'first_aider';
    }
    ...
    const victim = {
      ...
      isVictim: true,
      reachedGoal: false,
      helpNeeded: helpNeeded // Add this property to track what help they need
    };
    victims.push(victim);
    agents.push(victim);
  }

// This is the modified section from the generateRandomAgents function
// where we assign helpers to victims based on matching help type first

// Group victims by the type of help they need
const victimsByHelpNeeded = {
  packing: victims.filter(v => v.helpNeeded === 'packing'),
  first_aider: victims.filter(v => v.helpNeeded === 'first_aider')
};

// Group helpers by their type
const helpersByType = {
  packing: helpers.filter(h => h.type === 'packing'),
  first_aider: helpers.filter(h => h.type === 'first_aider')
};

console.log(`=== HELP NEEDS DISTRIBUTION ===`);
console.log(`Victims needing packing help: ${victimsByHelpNeeded.packing.length}`);
console.log(`Victims needing first aid: ${victimsByHelpNeeded.first_aider.length}`);
console.log(`Packing helpers available: ${helpersByType.packing.length}`);
console.log(`First aid helpers available: ${helpersByType.first_aider.length}`);

// Process each helper type separately
for (const helperType of Object.keys(helpersByType)) {
  // Get all helpers of this type
  const typeHelpers = [...helpersByType[helperType]];
  
  // Get all victims needing this type of help
  const typeVictims = [...victimsByHelpNeeded[helperType]];
  
  // For each helper of this type, find the closest victim needing this type of help
  for (let i = 0; i < typeHelpers.length; i++) {
    const helper = typeHelpers[i];
    
    // Skip if no matching victims left
    if (typeVictims.length === 0) continue;
    
    // Calculate distances from this helper to all matching victims
    const victimDistances = typeVictims.map(victim => {
      return {
        victim: victim,
        distance: calculateDistance(helper, victim)
      };
    ...
    // Assign this helper to the nearest matching victim
    ...
    const index = typeVictims.indexOf(nearestVictim);
    if (index > -1) {
      typeVictims.splice(index, 1);
    }
    
    console.log(`Assigned ${helper.type} helper to ${nearestVictim.type} victim (distance: ${victimDistances[0].distance.toFixed(2)})`);
  }
}

// Handle any remaining unassigned helpers and victims
const unassignedHelpers = helpers.filter(h => !h.assignedVictim);
const unassignedVictims = victims.filter(v => !v.assignedHelper);

if (unassignedHelpers.length > 0 && unassignedVictims.length > 0) {
  console.log(`NOTE: Assigning ${unassignedHelpers.length} remaining helpers to ${unassignedVictims.length} unmatched victims based on proximity`);
  
  // Assign remaining helpers to any victims (not matched by type)
  for (let i = 0; i < unassignedHelpers.length; i++) {
    const helper = unassignedHelpers[i];
    
    // Skip if no unassigned victims left
    if (unassignedVictims.length === 0) break;
    
    // Calculate distances from this helper to all unassigned victims
    const victimDistances = unassignedVictims.map(victim => {
      return {
        victim: victim,
        distance: calculateDistance(helper, victim)
      };
    ...
    // Assign this helper to the nearest unassigned victim
    const nearestVictim = victimDistances[0].victim;
    helper.assignedVictim = nearestVictim;
    nearestVictim.assignedHelper = helper;
    
    // Remove the assigned victim from the pool
    const index = unassignedVictims.indexOf(nearestVictim);
    if (index > -1) {
      unassignedVictims.splice(index, 1);
    }
    
    console.log(`Assigned remaining ${helper.type} helper to ${nearestVictim.type} victim needing ${nearestVictim.helpNeeded} (suboptimal match, distance: ${victimDistances[0].distance.toFixed(2)})`);
  }
}
...
// Now update the renderAgents function to show visual indicators
function renderAgents() {
  surface.selectAll("image.agent").remove();
  surface.selectAll("circle.helper-indicator").remove(); 
  surface.selectAll("circle.victim-indicator").remove(); // Add this line
  ...
  surface.selectAll("line.assignment")
    ...
    .attr("stroke", d => {
      // Color line based on helper type
      if (d.type === "packing") return "rgba(54, 162, 235, 0.7)";  // Blue for packing
      else return "rgba(255, 99, 132, 0.7)";  // Red for first aider
    })
    .attr("stroke-width", 2.5)
    .attr("stroke-dasharray", "5,3"); // Dashed line for better visibility
  
    
  // Draw NEW victim need indicators
  surface.selectAll("circle.victim-indicator")
    ...
    .attr("stroke-dasharray", d => {
      // Use solid line for victims with matched helpers, dashed for mismatched
      if (!d.assignedHelper) return "3,3";  // Dashed for unassigned 
      return d.assignedHelper.type === d.helpNeeded ? "none" : "3,3";  // Solid if matched, dashed if mismatched
    });
...
// Update logAssignmentDetails to remove urgency grouping
function logAssignmentDetails() {
  ...
  console.log("=== HELPER TYPE DISTRIBUTION ===");
  console.log(`Packing helpers: ${agents.filter(a => a.isHelper && a.type === 'packing').length}`);
  console.log(`First aid helpers: ${agents.filter(a => a.isHelper && a.type === 'first_aider').length}`);
  ...
  helpers.forEach((helper, index) => {
    if (helper.assignedVictim) {
      ...
      const isMatch = helper.type === helper.assignedVictim.helpNeeded;
      console.log(`Helper ${index+1} (${helper.type}) → Victim type: ${helper.assignedVictim.type}, Needs: ${helper.assignedVictim.helpNeeded}, Match: ${isMatch ? 'YES' : 'NO'}, Distance: ${distance}`);
    } else {
  ...
  // Count and log mismatched assignments
  const mismatchedAssignments = agents.filter(a => 
    a.isVictim && a.assignedHelper && a.helpNeeded !== a.assignedHelper.type
  );
  if (mismatchedAssignments.length > 0) {
    console.warn(`NOTE: ${mismatchedAssignments.length} victims have mismatched helper types`);
  }
}


// Update updateCounters to show helper types instead of urgency levels
function updateCounters() {
  ...
  // Update progress bars to show helpers instead of urgency stats
  const packingHelpers = {
    saved: savedCount.packing,
    total: agents.filter(a => a.isHelper && a.type === 'packing').length
  };
  
  const firstAidHelpers = {
    saved: savedCount.first_aider,
    total: agents.filter(a => a.isHelper && a.type === 'first_aider').length
  };
  ...
  document.getElementById('urgency-stats').innerHTML = `
    ...
    <div class="urgency-progress">
      <div>Packing Helpers: ${packingHelpers.saved}/${packingHelpers.total} (${packingHelpers.total > 0 ? Math.round(packingHelpers.saved/packingHelpers.total*100) : 0}%)</div>
      <div class="progress-bar">
        <div class="progress-fill" style="width: ${packingHelpers.total > 0 ? Math.round(packingHelpers.saved/packingHelpers.total*100) : 0}%; background-color: rgba(54, 162, 235, 0.7);"></div>
      </div>
    </div>
    <div class="urgency-progress">
      <div>First Aid Helpers: ${firstAidHelpers.saved}/${firstAidHelpers.total} (${firstAidHelpers.total > 0 ? Math.round(firstAidHelpers.saved/firstAidHelpers.total*100) : 0}%)</div>
      <div class="progress-bar">
        <div class="progress-fill" style="width: ${firstAidHelpers.total > 0 ? Math.round(firstAidHelpers.saved/firstAidHelpers.total*100) : 0}%; background-color: rgba(255, 99, 132, 0.7);"></div>
      </div>
    </div>
  `;
}
...
// Update showResults function to remove urgency levels and focus on helper types
function showResults() {
  ...
  // Calculate helper effectiveness
  const packingHelpers = {
    saved: savedCount.packing,
    total: agents.filter(a => a.isHelper && a.type === 'packing').length
  };
  ...
  modalContent.innerHTML = `
    ...
    <div class="stat-box">Youth: ${results.youth} (${totalAgentsByType.youth - results.youth} perished)</div>
    <div class="stat-box">Adults: ${results.adult} (${totalAgentsByType.adult - results.adult} perished)</div>
    <div class="stat-box">Elderly: ${results.elderly} (${totalAgentsByType.elderly - results.elderly} perished)</div>
    <div class="stat-box">PWD: ${results.disabled} (${totalAgentsByType.disabled - results.disabled} perished)</div>
    
    <h3 style="color:#2c3e50; margin-top:20px;">Helper Effectiveness</h3>
    <div class="urgency-progress">
      <div>Packing Helpers: ${packingHelpers.saved}/${packingHelpers.total} (${packingHelpers.total > 0 ? Math.round(packingHelpers.saved/packingHelpers.total*100) : 0}%)</div>
      <div class="progress-bar">
        <div class="progress-fill" style="width: ${packingHelpers.total > 0 ? Math.round(packingHelpers.saved/packingHelpers.total*100) : 0}%; background-color: rgba(54, 162, 235, 0.7);"></div>
