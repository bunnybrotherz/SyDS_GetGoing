...
// Function to generate victims with the specified demographics
function generateVictimsWithDemographics() {
  ...
  const scaledYouth = demographics.youth;
  const scaledAdult = demographics.adult;
  const scaledElderly = demographics.elderly;
  const scaledDisabled = demographics.disabled;
  
  
  console.log(`Scaled demographics - Youth: ${scaledYouth}, Adults: ${scaledAdult}, Elderly: ${scaledElderly}, Disabled: ${scaledDisabled}`);

  // Create youth victims
  for (let i = 0; i < scaledYouth; i++) {
    positions.push({
  ...
  // Create adult victims
  for (let i = 0; i < scaledAdult; i++) {
    positions.push({
      row: Math.floor(Math.random() * maxRows) + 1,
      col: Math.floor(Math.random() * maxCols) + 1,
      type: 'adult',
  ...
  // Create elderly victims
  for (let i = 0; i < scaledElderly; i++) {
    positions.push({
      row: Math.floor(Math.random() * maxRows) + 1,
      col: Math.floor(Math.random() * maxCols) + 1,
      type: 'elderly',
  ...
  // Create disabled victims
  for (let i = 0; i < scaledDisabled; i++) {
    positions.push({
      row: Math.floor(Math.random() * maxRows) + 1,
      col: Math.floor(Math.random() * maxCols) + 1,
      type: 'disabled',
...
function setupCanvas() {
  ...
  counterContainer.innerHTML = `
    ...
    <p id="youth-count">Youth Saved: 0 / ${demographics.youth}</p>
    <p id="adult-count">Adults Saved: 0 / ${demographics.adult}</p>
    <p id="elderly-count">Elderly Saved: 0 / ${demographics.elderly}</p>
    <p id="disabled-count">Disabled Saved: 0 / ${demographics.disabled}</p>
...
function resetSimulation() {
  ...
  // Generate new victims based on demographics
...
function storeResults(runNumber) {
  ...
  const youthPercent = ((savedCount.youth / demographics.youth) * 100).toFixed(1);
  const adultPercent = ((savedCount.adult / demographics.adult) * 100).toFixed(1);
  const elderlyPercent = ((savedCount.elderly / demographics.elderly) * 100).toFixed(1);
  const disabledPercent = ((savedCount.disabled / demographics.disabled) * 100).toFixed(1);
  ...
  const result = {
    ...
    youthPercent: youthPercent,
    adultPercent: adultPercent,
    elderlyPercent: elderlyPercent,
