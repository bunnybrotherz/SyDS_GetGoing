...
function updateAgents() {
  ...
  agents.forEach((a) => {
    if (a.assignedVictim && !a.reachedGoal && !a.assignedVictim.reachedGoal) {
      ...
      if (!reachedVictim) {
        moveTowards(a, a.assignedVictim); // Move towards victim
      } else {
...
function generateRandomAgents(count) {
  ...
  for (let i = 0; i < helpersCount; i++) {
    const helperType = Math.random() < 0.5 ? 'packing' : 'first_aider';
    positions.push({
      row: Math.floor(Math.random() * maxRows) + 1,
      col: Math.floor(Math.random() * maxCols) + 1,
      type: 'helper',
      helperRole: helperType,  // new field
      waitTime: helperType === 'packing' ? 5 : 15,  // in seconds
      waiting: false,  // flag for wait state
      waitStartTime: null
    });
  }
