...
// CHANGE 3: Simplified urgency levels - just High and Low
var urgencyLevels = {
  youth: 1,    // High urgency
  adult: 1,    // High urgency
  elderly: 2,  // Low urgency (changed from 3 to 2)
  disabled: 2  // Low urgency (changed from 2 to 2)
};
...
function runSimulation(simulationSpeedMultiplier = 1000) {
  ...
  while (simulationTime < maxSimulationTime && !areAllVictimsEscorted()) {
    ...
    // CHANGE 2: Check for helpers who can be reassigned
    reassignHelpers();
...
// CHANGE 2: Function to check if all victims are either escorted to safety or have no chance of being rescued
...
// CHANGE 2: Function to reassign helpers who have completed their task
...
function updateAgents(deltaTime) {
  agents.forEach((agent) => {
    ...
    // CHANGE 1: Unassisted victims can't move - completely removed the else clause for victims without helpers
  });
}
...
// CHANGE 2: Modified to mark agents as permanently at goal or just reached goal
...
function generateRandomAgents() {
  ...
  for (let i = 0; i < totalHelpers; i++) {
    ...
    const helper = {
      ...
      isHelper: true,
      reachedGoal: false,
      permanentlyAtGoal: false, // CHANGE 2: Added to track if helper is done for good
      counted: false,
      victimsHelped: 0
    };
  ...
  victimGroups.forEach(group => {
    for (let i = 0; i < group.count; i++) {
      ...
      const victim = {
        ...
        isVictim: true,
        reachedGoal: false,
        permanentlyAtGoal: false, // CHANGE 2: Added for consistency
        counted: false,
  ...
  // CHANGE 3: Modified to use the simplified priority system
