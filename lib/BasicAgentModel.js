...
document.getElementById("test-button").addEventListener("click", () => {
  ...
  perishedCount = { youth: 0, adult: 0, elderly: 0, disabled: 0 }; // Reset perished count
...
// Add a function to calculate perished people
...
// Add a function to calculate efficiency score
...
// Update the rendering for agents - UPDATED: center the images within urgency circles
function renderAgents() {
  ...
  // UPDATED: position adjusted to center in the urgency circles
...
// Update the showResults function to include perished count and efficiency
function showResults() {
  ...
  modalContent.innerHTML = `
    ...
    <h3 style="color:#2c3e50;">Overall Efficiency: ${efficiencyScore}%</h3>
    <div class="progress-bar" style="margin-bottom: 20px;">
      <div class="progress-fill" style="width: ${efficiencyScore}%; background-color: #3498db;"></div>
    </div>
    ...
    <div class="stat-box low-urgency">Elderly: ${results.elderly} (${totalAgentsByType.elderly - results.elderly} perished)</div>
    ...
    <h3 style="color:#2c3e50; margin-top:20px;">Summary of Perished Victims</h3>
    <div>Youth: ${perished.youth}</div>
    <div>Adults: ${perished.adult}</div>
    <div>Elderly: ${perished.elderly}</div>
    <div>Disabled: ${perished.disabled}</div>
...
// Update the test button event listener to also reset total counts
document.getElementById("test-button").addEventListener("click", () => {
  cleanupTimers();
  timer = 30;
  savedCount = { youth: 0, adult: 0, elderly: 0, disabled: 0, packing: 0, first_aider: 0 };
