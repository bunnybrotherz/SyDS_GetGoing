...
var victimImages = {
  ...
  helper: "images/previous.png" // New helper image
};
...
// Function to render agents (victims and helpers)
...
function updateAgents() {
  agents.forEach((v, index) => {
    let speed = victimSpeeds[v.type];  // Get the speed for each agent type
    ...
    if (Math.abs(v.row - goal.row) < epsilon && Math.abs(v.col - goal.col) < epsilon) {
      ...
      // Increment saved count for the agent's type
      savedCount[v.type]++;
...
function setupCanvas() {
  ...
  agents = generateRandomAgents(100);  // Generate 100 agents based on the ratio
