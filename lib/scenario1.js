...
var isRunning = false;
var timer = 30;
var savedCount = { youth: 0, adult: 0, elderly: 0, PWD: 0 };
...
var urgencyLevels = {
  ...
  PWD: 2  // Medium urgency
};

// Total victims by type for statistics
var totalVictimsByType = { youth: 0, adult: 0, elderly: 0, PWD: 0 };
...
function generateRandomVictims(count) {
  ...
  // Reset total counts
  totalVictimsByType = { youth: 0, adult: 0, elderly: 0, PWD: 0 };
  ...
  console.log(`PWD victims: ${victims.filter(v => v.type === 'PWD').length}`);
  ...
  surface.selectAll("circle.urgency")
    ...
    .attr("fill", d => {
      ...
      if (d.hasDelay && Date.now() - d.spawnTime < 20000 / speedMultiplier) {
        ...
        return "rgba(255, 159, 64, 0.3)"; // Elderly and PWD
      }
    });
...
function startButtonHandler() {
  cleanupTimers();
  timer = 30;
  savedCount = { youth: 0, adult: 0, elderly: 0, PWD: 0 };
  perishedCount = { youth: 0, adult: 0, elderly: 0, PWD: 0 };
...
function showResults() {
  ...
  modalContent.innerHTML = `
    ...
    <div class="result-category">
      <div class="result-label">PWD: ${results.PWD}/${totalVictimsByType.PWD}</div>
      <div class="result-bar-container">
